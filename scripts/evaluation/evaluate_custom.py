...
def load_and_split_dataset(backtest_config: dict):
    path = backtest_config["path"]
    prediction_length = backtest_config["prediction_length"]
    freq = backtest_config.get("freq", "H")  # Assume hourly unless specified
    offset = backtest_config.get("offset", -prediction_length)
    num_rolls = backtest_config.get("num_rolls", 1)
    column = backtest_config.get("column", "value")

    # Load single-column dataset
    df = pd.read_excel(path, sheet_name=1)

    # Synthesize timestamps
    start = pd.Timestamp("2020-01-01 00:00:00")  # arbitrary start
    timestamps = pd.date_range(start=start, periods=len(df), freq=freq)

    target_series = df[column].values
    ...
    _, test_template = split(series, offset=offset)
